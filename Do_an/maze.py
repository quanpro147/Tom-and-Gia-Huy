...
import json
#Check xem tai khoan đã tồn tại chưa
def checkaccount(current_acc, currenct_pass):
    with open("account.txt","r") as f:
        data = json.load(f)
        for user in data["users"]:
            if user["username"] == current_acc and user["password"] == currenct_pass:
                return True
    return False
class LoginWidget(QWidget):
    ...
    def register(self):
        ...
        if checkaccount(username, password):
            QMessageBox.warning(self, "Tài khoản đã tồn tại", "Hãy dùng sử dụng tên đăng nhập khác")
        else:
            with open("account.txt", "r") as fin:
                data = json.load(fin)
                data["users"].append({"username": username, "password": password})
            with open("account.txt", "w") as fout:
...
if __name__ == "__main__":
    ...
    login.show()
    sys.exit(app.exec_())
>>>>>>> b5ebe9f5e4e6b9f7105c91b0320cb8bc0f6bdf14:Do_an/login.py
